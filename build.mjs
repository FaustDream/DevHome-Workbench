/**
 * Thrilled 构建脚本
 * - 将 src/background、src/pages/index、src/pages/popup 的 TS 入口用 esbuild 打包为 dist/ 产物
...
const entries = [
  ...
  { in: join(ROOT, 'src/pages/popup.ts'), out: join(DIST, 'popup.js') },
];
...
async function main() {
  ...
  for (const file of ['index.html', 'popup.html', 'sidepanel.html', 'manifest.json', 'defaults.json']) {
